    ...
    return (
        <div className="flex gap-6 h-[calc(100vh-10rem)]">
            ...
            <div className="w-64 flex-shrink-0 hidden lg:flex flex-col gap-3">
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Active Papers</p>
                <div className="space-y-2 overflow-y-auto flex-1">
                    {MOCK_PAPERS.map(paper => (
                        <button key={paper.id} onClick={() => togglePaper(paper.id)}
                            className={`w-full text-left p-3 rounded-xl border text-xs transition-all ${selectedPapers.includes(paper.id)
                                ? "bg-teal-500/10 border-teal-500/25 text-teal-300"
                                : "bg-slate-900 border-white/[0.06] text-slate-400 hover:border-white/[0.12]"
                                }`}>
                            <div className="flex items-start gap-2">
                                <div className={`w-4 h-4 rounded flex-shrink-0 mt-0.5 flex items-center justify-center text-xs border ${selectedPapers.includes(paper.id)
            ...
            {/* Chat Area */}
            <div className="flex-1 flex flex-col bg-slate-900 border border-white/[0.06] rounded-2xl overflow-hidden">
                {/* Header */}
                <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400">◉</div>
                    <div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    {messages.map((msg, i) => (
                        <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                            {msg.role === "assistant" && (
                                <div className="w-7 h-7 rounded-lg bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400 text-xs flex-shrink-0 mr-3 mt-1">AI</div>
                            )}
                            <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.role === "user"
                                ? "bg-teal-600 text-white"
                                : "bg-slate-800 border border-white/[0.06] text-slate-200"
                                }`}>
                    ...
                    {loading && (
                        <div className="flex items-center gap-3">
                            <div className="w-7 h-7 rounded-lg bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400 text-xs flex-shrink-0">AI</div>
                            <div className="bg-slate-800 border border-white/[0.06] rounded-2xl px-4 py-3">
                ...
                <div className="p-4 border-t border-white/[0.06]">
                    <div className="flex gap-2">
                        <input value={input}
                            ...
                            className="flex-1 bg-slate-800 border border-white/[0.08] text-white rounded-xl px-4 py-3 text-sm placeholder-slate-500 focus:outline-none focus:border-teal-500/50 transition-colors" />
                        <button onClick={send}
                            className="bg-teal-600 hover:bg-teal-500 text-white px-4 rounded-xl transition-colors text-sm">→</button>
                    </div>
