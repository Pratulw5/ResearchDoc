...
export default function ComparePage() {
    ...
    const data: Record<number, string[]> = {
        ...
        2: ["Transformer (encoder)", "110M / 340M", "BooksCorpus + Wikipedia", "N/A", "512 tokens", "Yes", "Medium"],
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {MOCK_PAPERS.map(paper => (
                        <button key={paper.id}
                            onClick={() => setSelected(prev => prev.includes(paper.id) ? prev.filter(id => id !== paper.id) : [...prev, paper.id])}
                            className={`p-3 rounded-xl border text-left transition-all text-sm ${selected.includes(paper.id) ? "bg-indigo-500/10 border-indigo-500/40" : "bg-slate-800 border-slate-700 hover:border-slate-500"}`}>
                            <p className={`font-medium text-xs leading-snug ${selected.includes(paper.id) ? "text-indigo-300" : "text-slate-300"}`}>{paper.title.slice(0, 50)}...</p>
                            <p className="text-slate-500 text-xs mt-1">{paper.year}</p>
                        </button>
                    ))}
                </div>
                <button onClick={generate} disabled={selected.length < 2 || generating}
                    className="mt-4 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition-colors flex items-center gap-2">
                    {generating ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Generating...</> : "⊟ Generate Comparison"}
                </button>
            </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <button className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg transition-colors border border-slate-700">Export CSV</button>
                            <button className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg transition-colors border border-slate-700">Google Sheets</button>
                        </div>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-slate-800">
                                    <th className="text-left text-xs font-semibold text-slate-400 px-5 py-3 uppercase tracking-wide">Attribute</th>
                                    {papers.map(p => (
                                        <th key={p.id} className="text-left text-xs font-semibold text-slate-300 px-5 py-3">
                                            <div className="max-w-[160px] truncate">{p.title.split(":")[0]}</div>
                            ...
                            <tbody>
                                {attrs.map((attr, i) => (
                                    <tr key={attr} className={`border-b border-slate-800/50 ${i % 2 === 0 ? "" : "bg-slate-800/20"}`}>
                    ...
                    <p className="px-5 py-3 text-xs text-slate-600 border-t border-slate-800">Auto-extracted by AI · Click any cell to edit · Data sourced from papers</p>
                </div>
            )}
        </div>
    );
